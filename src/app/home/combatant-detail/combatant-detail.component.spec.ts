...
describe('CombatantDetailComponent', () => {
  ...
  describe('the language table', () => {
    ...
    it('should render a sorted table', () => {
      ...
      expect(tableBody.querySelector('tr:first-child td:last-child').innerHTML).toBe(
        dataWithLangCount.repos.languages.first.toString()
      );
      expect(tableBody.querySelector('tr:nth-child(2) td:first-child').innerHTML).toBe('middle');
      expect(tableBody.querySelector('tr:nth-child(2) td:last-child').innerHTML).toBe(
        dataWithLangCount.repos.languages.middle.toString()
      );
      expect(tableBody.querySelector('tr:last-child td:first-child').innerHTML).toBe('last');
      expect(tableBody.querySelector('tr:last-child td:last-child').innerHTML).toBe(
        dataWithLangCount.repos.languages.last.toString()
      );
    });
  });
});
